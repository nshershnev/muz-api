...
describe("Authorization", () => {
  ...
  describe("POST /api/v1/users", () => {
    it("should return User which was created", () => {
    ...
    it("should return Email is already used", () => {
      return request.post("/api/v1/users")
        .send(user)
        .then(({ body: { error } }) => {
          expect(error).toHaveProperty("message", "Email is already used");
        });
    });
  ...
  describe("POST /api/v1/login", () => {
    ...
    it("should return Incorrect username or password", () => {
      return request.post("/api/v1/login")
        ...
        .then(({ body: { error } }) => {
          expect(error).toHaveProperty("message", "Incorrect username or password");
        });
    });
  });
